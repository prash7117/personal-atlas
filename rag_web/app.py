...
def _build_subprocess_env(openai_api_key: str) -> Dict[str, str]:
    """Build subprocess env with required OpenAI key."""
    env = os.environ.copy()
    env["OPENAI_API_KEY"] = openai_api_key
    return env


def _run_openai_connection_test(openai_api_key: str) -> Dict[str, Any]:
...
async def test_openai_connection(req: OpenAITestRequest) -> JSONResponse:
    ...
    try:
        result = await asyncio.to_thread(_run_openai_connection_test, candidate)
    except Exception as exc:  # pylint: disable=broad-exception-caught
    ...
    payload: Dict[str, Any] = {"ok": True, "source": source}
...
async def answer(req: AnswerRequest) -> JSONResponse:
    ...
    env = _build_subprocess_env(openai_api_key)
